        ...
        if (isTrackedEvent) {
          try {
            ...
            if (isRotate && isCurrent) {
              // The payload position is the first event of the NEW file:
              // the only value coherent with binlogName (the header
              // nextPosition refers to the OLD file, 0 for the artificial
              // rotate at dump start)
