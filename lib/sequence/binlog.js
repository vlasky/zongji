  ...
  class Binlog extends Command {
    ...
    binlogData(packet, connection) {
      ...
      if (this._callback) {
        ...
        // Commit markers also need parsing while a transaction's GTID is
        // in flight (_currentGtid set), so it can be cleared instead of
        // leaking onto unrelated events between transactions
        ...
        if (isTrackedEvent) {
          try {
            ...
            // transaction, so its GTID must not linger either
            if (isCurrent && (isGtidEvent || eventName === 'xid' ||
        ...
        if (zongji._skipEvent(eventName) && !error) {
          ...
          // the same rules as delivered ones (never past a TableMap, never
          // from a rotate header, never adopting MariaDB's zero
          // end_log_pos). On MariaDB only commits and standalone events
          // carry a real end position, so filtering out 'xid'/'query'
          // would otherwise freeze options.position at the start point.
          if (isCurrent && eventName !== 'tablemap' && !isRotate &&
              binlogPacket.nextPosition > 0) {
