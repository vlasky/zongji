...
// string, number, boolean or null). The caller decides whether to
// serialise back to a JSON string (mysql2 `jsonStrings` option).
...
function parseBinaryBuffer(input, offset, parentValueOffset, readUInt, isLarge) {
  ...
  switch (jsonType) {
    ...
    case JSONB_TYPE_INT64: {
      const low = input.readUInt32LE(valueOffset + 1);
      const high = input.readUInt32LE(valueOffset + 5);
      // Number when within the safe integer range, exact string otherwise
      result = common.signedInt64ToNumberOrString(high, low);
      break;
    }
    case JSONB_TYPE_UINT64: {
      const low = input.readUInt32LE(valueOffset + 1);
      const high = input.readUInt32LE(valueOffset + 5);
      result = common.unsignedInt64ToNumberOrString(high, low);
      break;
    }
