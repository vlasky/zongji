  ...
  )
{
  ...
  switch (column.type) {
    ...
    case MysqlTypes.VARCHAR_COMPRESSED: {
      ...
      if (columnSchema.COLUMN_TYPE.substr(0, 6) === 'varbin') {
        result = buffer;
      } else {
        let charsetName = column.charset || 'utf8';
        if (charsetName === 'utf8mb3' || charsetName === 'utf8mb4') {
          charsetName = 'utf8';
        }
        result = iconv.decode(buffer, charsetName);
      }
      break;
    }
