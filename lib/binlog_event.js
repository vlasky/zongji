...
class TableMap extends BinlogEvent {
  ...
  _readOptionalMetadata(parser, zongji) {
    ...
    while (parser._offset < end) {
      ...
      // A field parser reading past its own length means the block is
      // corrupt; treat all optional metadata as unusable rather than
      // trusting partially garbled values (handled by the caller's catch)
      if (parser._offset > fieldEnd) {
        throw new Error('Malformed TABLE_MAP_EVENT optional metadata');
      }
