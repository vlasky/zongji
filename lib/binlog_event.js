...
/* MariaDB compressed event variants (codes 165-171), written when
 * log_bin_compress=ON. The envelope is decodable (zlib), but zongji does
 * not implement it yet; warn loudly instead of dropping silently.
 */
class QueryCompressed extends BinlogEvent {
  constructor(parser, options, zongji) {
    super(parser, options);
    skipEventRemainder(parser, zongji);
  }
  ...
  static unsupportedReason =
    'Server sent a QUERY_COMPRESSED_EVENT (log_bin_compress=ON). zongji ' +
    'cannot decode compressed binlog events yet, so these statements are ' +
    'NOT being emitted. Set log_bin_compress=OFF on the server to receive ' +
    'them.';
}

class RowsCompressed extends BinlogEvent {
  constructor(parser, options, zongji) {
    super(parser, options);
    skipEventRemainder(parser, zongji);
  }
  ...
  static unsupportedReason =
    'Server sent a compressed row event (log_bin_compress=ON). zongji ' +
    'cannot decode compressed binlog events yet, so these row changes are ' +
    'NOT being emitted. Set log_bin_compress=OFF on the server to receive ' +
    'them.';
}
