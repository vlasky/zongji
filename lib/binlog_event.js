...
import { collationToCharset } from './charset_map.js';
...
class TableMap extends BinlogEvent {
    ...
    while (parser._offset < end) {
      ...
      switch (type) {
        ...
        case Types.COLUMN_VISIBILITY:
          ...
          break;
        // GEOMETRY_TYPE and unknown future fields (e.g. VECTOR
        // dimensionality in MySQL 9) are skipped via fieldEnd below
      }
    ...
    for (let i = 0; i < this.columnCount; i++) {
      ...
      switch (type) {
        ...
        case Mysql.VAR_STRING: {
          // The byte width is exact for binary columns (and required by the
          // decoder); for text columns the character width cannot be
          // recovered from bytes, so no width is synthesised
        ...
        case Mysql.BLOB:
          schema.COLUMN_TYPE = binary ? 'blob' : 'text';
          break;
        case Mysql.ENUM:
        case Mysql.SET: {
        ...
        case Mysql.GEOMETRY: schema.COLUMN_TYPE = 'geometry'; break;
  ...
  updateColumnInfo() {
    ...
    // signedness; prefer it over inferring from the INFORMATION_SCHEMA
    // column definition (see parseAnyInt in common.js)
    if (this.signedness && columnSchemas.length === this.columnCount) {
      for (let i = 0; i < this.columnCount; i++) {
        if (this.signedness[i] !== undefined &&
            columnSchemas[i].UNSIGNED === undefined) {
