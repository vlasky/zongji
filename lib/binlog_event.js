...
class Gtid extends BinlogEvent {
  ...
  _parseTagged(parser, zongji) {
    ...
    const varint = () => Common.parseSerializationVarint(parser, end, what);
    const version = varint();
    if (version !== 1n) {
      throw new Error(
        `Unsupported tagged GTID serialization version ${version}`);
    }
    const encodedSize = Number(varint());
    ...
    parser.parseUnsignedNumber(1);
    const gno = Common.parseSerializationVarintSigned(parser, end, what);
...
class PreviousGtids extends BinlogEvent {
  constructor(parser, options, zongji) {
    ...
    this.sids = sids;
    // Entries arrive sorted by (uuid, tag) with the untagged entry
    // first, so consecutive entries sharing a uuid print as one block:
    // 'uuid:1-5:tag_a:1:tag_b:1', matching @@gtid_executed
    let text = '';
    let lastSid = null;
    for (const entry of sids) {
      const ranges = entry.intervals.map(interval =>
        interval.start === interval.end - 1 ?
          `${interval.start}` :
          `${interval.start}-${interval.end - 1}`);
      if (entry.sid === lastSid) {
        text += `:${entry.tag}:${ranges.join(':')}`;
      } else {
        text += (text ? ',' : '') + entry.sid +
          (entry.tag ? `:${entry.tag}` : '') + `:${ranges.join(':')}`;
        lastSid = entry.sid;
      }
    }
    this.gtidSet = text;
